...
//信号定义
reg [ 3:0]   init_state   ;//一段式状态机跳转
reg [ 6:0]   cmd_index    ;
reg [31:0]   clk_cnt      ;
reg [ 4:0]   bit_loop     ;
reg [15:0]   pixel_cnt    ;//像素点计数器
reg          cs_r         ;
reg          dc_r         ;
reg          lcd_reset_r  ;
reg [7:0]    spi_data     ;
...
assign res   = lcd_reset_r;
assign sclk  = ~clk_50m   ;
assign cs    = cs_r       ;
assign dc    = dc_r       ;
assign sda   = spi_data[7]; // MSB
...
// 					(pixel_cnt >= 10800) ? 16'h07E0 : 16'h001F;
	...
	always@(*)begin
		if(key_data==0)begin
			pixel = (pixel_cnt >= 21600) ? 16'hF800 :((pixel_cnt >= 10800) ? 16'h07E0 : 16'h001F);
		end
			...
			pixel = (pixel_cnt >= 21600) ? 16'h001F :((pixel_cnt >= 10800) ? 16'hF800 : 16'h07E0);
		end
	end
